...
class RemoteDateInput(RemoteTimeInput):
    def as_dict(self):
        ...
        widget_dict["choices"] = [
            ...
            {"title": "month", "data": [{"key": x, "value": y} for (x, y) in list(MONTHS.items())]},
...
class RemoteSelect(RemoteWidget):
    def as_dict(self):
        ...
        for key, value in self.widget.choices:
            widget_dict["choices"].append({"value": key, "display": value})
...
class RemoteRadioSelect(RemoteSelect):
    def as_dict(self):
        ...
        widget_dict["choices"] = []
        for key, value in self.widget.choices:
            widget_dict["choices"].append({"name": self.field_name or "", "value": key, "display": value})
